...
var charsMultiplesFound = ""

fun main(args: Array<String>) {
    val lines = input02a2.lines()
    for (line in lines) {
        line.toCharArray()
        val frequenciesByChar = line.groupingBy { it }.eachCount()
        //println(frequenciesByChar)
        var tmpCharsMultFound = ""
        for (freqs in frequenciesByChar) {

            if (freqs.value > 1) {
                println("Value: " + freqs.value.toString() + " tmpFound: " + tmpCharsMultFound)
                if (freqs.value.toString() in tmpCharsMultFound) break
                println(freqs)
                tmpCharsMultFound += freqs.value
            }
        }
        charsMultiplesFound += tmpCharsMultFound
    }
    println("Findings: " + charsMultiplesFound)

    val frequenciesByChar2 = charsMultiplesFound.groupingBy { it }.eachCount()
    println(frequenciesByChar2)
    for (freqs in frequenciesByChar2) {
        if (freqs.value > 1) {
            println(freqs)
            charsMultiplesFound += freqs.key
